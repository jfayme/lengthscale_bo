...
ROW_FIELDS = (
    ...
    "family", "timestamp", "lsab_commit", "torch_threads", "omp_threads",
    # cell
...
def run_task(task: Task, *, n_init: int, n_iter: int, seed_base: int, device: str = "cpu",
    ...
    row.update(family=fp.pool.spec.family, lsab_commit=_commit(), torch_threads=torch.get_num_threads(),
               omp_threads=os.environ.get("OMP_NUM_THREADS", ""), n=fp.pool.n, d=fp.d,
...
def sweep(tasks, out_path, *, n_init, n_iter, seed_base, include_degenerate=False, device="cpu",
          cache_dir=None, dry_run=False, done_from=None, allow_embedding=False) -> int:
    ...
    if missing and not allow_embedding:
        ...
        print("embed them first (one process), then rerun:\n  python -m lsab.featurize --rep "
    ...
    for task in todo:
        ...
        if built[0] != (task.seed, task.cell):   # one cell in memory: the 10k pools are large
            ...
            except Exception as error:           # skip the cell; do not lose the rest of the shard
    ...
    if broken:
        print(f"{written} rows written; {len(broken)} cell(s) could not be built and were skipped:")
...
def _dry_run(todo, files, missing, device, cache_dir) -> None:
    ...
    for cell in remaining:
        if unembedded[(cell.dataset, cell.rep)]:
            shape = (f"{unembedded[(cell.dataset, cell.rep)]} molecules not embedded yet "
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument("--threads", type=int, default=1, help="torch AND BLAS threads; run k shards on k cores instead")
    ...
    if args.allow_embedding and args.shard:
        parser.error("--allow-embedding cannot be combined with --shard: shards must never write the embedding cache")
    ...
    print(f"threads: torch {torch.get_num_threads()}, OMP_NUM_THREADS={os.environ.get('OMP_NUM_THREADS', 'unset')}  "
          f"output: {out}", flush=True)
    sweep(tasks, out, include_degenerate=args.include_degenerate, device=args.device, cache_dir=args.cache_dir,
          dry_run=args.dry_run, done_from=files + ([out] if out not in files else []),
          allow_embedding=args.allow_embedding, **design)
