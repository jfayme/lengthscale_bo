...
exactly that header or the run refuses before writing a byte. A failed campaign is a
row, not a crash; a degenerate cell is skipped with one printed line.
...
from lsab import featurize
from lsab.bo import Campaign, CampaignError, initial_design, metrics, run_campaign
from lsab.datasets import DEFAULT_DATASETS, load
from lsab.lengthscale import make_prior, pool_geometry, rule_chen, rule_geom
from lsab.reduce import build

ROW_FIELDS = (
    # key: identifies a task (KEY_FIELDS is this prefix)
    "dataset", "reduction", "rep", "rule", "prior_mode", "cv", "seed", "n_init", "n_iter", "seed_base",
    # provenance
    "family", "timestamp", "lsab_commit",
    # cell
...
# 1. THE TASK LIST
# =============================================================================
def task_list(datasets, reductions, reps, rules, prior_modes, cv, seeds, seed_offset) -> list[Task]:
    """Seed-major: for seed, for cell (dataset, reduction, rep), for arm (rule, prior_mode)."""
    cells = [CellKey(d, red, rep) for d in datasets for red in reductions for rep in reps]
    arms = [Arm(rule, mode, float(cv)) for rule in rules for mode in prior_modes]
    return [Task(seed, cell, arm) for seed in range(seed_offset, seed_offset + seeds)
...
def row_key(task: Task, n_init: int, n_iter: int, seed_base: int) -> tuple:
    ...
    return (c.dataset, c.reduction, c.rep, a.rule, a.prior_mode, float(a.cv), int(task.seed),
            int(n_init), int(n_iter), int(seed_base))
...
def _records(path) -> list[dict]:
    ...
    with open(path, newline="", encoding="utf-8") as handle:
        ...
        if header != ROW_FIELDS:
            at = next((i for i, (a, b) in enumerate(zip(header, ROW_FIELDS)) if a != b),
            ...
            raise ValueError(f"{path}: header differs from ROW_FIELDS at column {at}: expected "
                             f"{expected!r}, found {found!r}. Refusing to read or append to it.")
        return [dict(zip(ROW_FIELDS, r)) for r in rows if len(r) == len(ROW_FIELDS)]   # a torn line is not done


def read_done_keys(path) -> set[tuple]:
    """Keys of the complete rows in `path`; set() if it does not exist; raises on a header mismatch."""
    return {tuple(kind(r[f]) for kind, f in zip(_KEY_TYPES, KEY_FIELDS)) for r in _records(path)}
...
def _append(path: Path, row: dict) -> None:
    if set(row) != set(ROW_FIELDS):
        raise ValueError(f"row fields differ from ROW_FIELDS: {sorted(set(row) ^ set(ROW_FIELDS))}")
...
def build_cell(cell: CellKey, *, device: str = "cpu", cache_dir=None):
    ...
    fp = build(load(cell.dataset), cell.rep, cell.reduction, device=device, cache_dir=cache_dir)
    return fp, pool_geometry(fp)


def run_task(task: Task, *, n_init: int, n_iter: int, seed_base: int, device: str = "cpu",
             cache_dir=None, built=None) -> dict:
    ...
    fp, g = built if built is not None else build_cell(task.cell, device=device, cache_dir=cache_dir)
    ...
    row.update(family=fp.pool.spec.family, lsab_commit=_commit(), n=fp.pool.n, d=fp.d,
               used_reps=";".join(f"{b.name}={b.used_rep}" for b in components),
               n_failed_embed=sum(b.n_failed for b in components), D_bar=g.mean, geom_cv=g.cv,
               degenerate=g.degenerate, ell_geom=rule_geom(g), ell_chen=rule_chen(g),
               ell_0=prior.ell_0, concentration=prior.concentration, rate=prior.rate)
    try:
        result = run_campaign(Campaign(fp, prior, n_init=n_init, n_iter=n_iter, seed=task.seed, seed_base=seed_base))
    except CampaignError as error:
        cause = error.__cause__ or error
        design = initial_design(fp.pool.n, n_init, task.seed, seed_base)
        row.update(dict.fromkeys(OUTCOME_FIELDS, math.nan), failed=True, fail_iteration=error.iteration,
                   error=f"{type(cause).__name__}: {cause}"[:200])
    else:
        design = result.sampled_indices[:n_init]
        row.update(metrics(result, fp.pool.objective, n_init), failed=False, fail_iteration=math.nan, error="")
    row.update(init_indices=";".join(str(int(i)) for i in design),
...
# =============================================================================
def sweep(tasks, out_path, *, n_init, n_iter, seed_base, include_degenerate=False, device="cpu",
          cache_dir=None, dry_run=False, done_from=None) -> int:
    """Run the tasks not yet in `done_from` (default: `out_path`), appending to `out_path`. Rows written."""
    ...
    print(f"tasks: {len(tasks)} in this run, {len(tasks) - len(todo)} done, {len(todo)} to run", flush=True)
    if dry_run:
        _dry_run(todo, done_from or [out_path], device, cache_dir)
        return 0
    written, built, skipped = 0, (None, None), set()
    for task in todo:
        if built[0] != (task.seed, task.cell):   # one cell in memory: the 10k pools are large
            built = ((task.seed, task.cell), build_cell(task.cell, device=device, cache_dir=cache_dir))
        if built[1][1].degenerate and not include_degenerate:
            if task.cell not in skipped:
                print(f"skip {_label(task.cell)}: degenerate pool (every distance equal)", flush=True)
                skipped.add(task.cell)
            continue
        row = run_task(task, n_init=n_init, n_iter=n_iter, seed_base=seed_base, built=built[1])
        _append(out_path, row)
        written += 1
        outcome = f"FAILED@{row['fail_iteration']}" if row["failed"] else f"auc={row['auc']:.3f}  {row['seconds']:.1f}s"
        print(f"seed={task.seed:<3} {_label(task.cell):40s} {task.arm.rule:5s} {task.arm.prior_mode:23s} "
              f"cv={task.arm.cv:g}  ell_0={row['ell_0']:<8.3g} {outcome}", flush=True)
    return written
...
def _dry_run(todo, files, device, cache_dir) -> None:
    ...
    for path in files:
        for r in _records(path):
            if r["failed"] == "False" and r["seconds"]:
                timings[(r["dataset"], r["reduction"], r["rep"])].append(float(r["seconds"]))
    remaining, total, untimed = Counter(t.cell for t in todo), 0.0, 0
    for cell in remaining:
        pool = load(cell.dataset)
        missing = sum(len(featurize.uncached(cell.rep, s, **({"cache_dir": cache_dir} if cache_dir else {})))
                      for s in pool.components.values())
        if missing:
            shape = f"{missing} molecules not embedded yet (python -m lsab.featurize --rep {cell.rep} --dataset {cell.dataset})"
        else:
            try:
            ...
            except Exception as error:   # a cell that cannot be built would stop the real run
        ...
        estimate = "?" if median is None else f"{median:.0f} s/task -> {median * remaining[cell] / 3600:.2f} h"
        print(f"  {_label(cell):40s} {remaining[cell]:>5} tasks  {estimate:22s} {shape}", flush=True)
    print(f"estimated wall time: {total / 3600:.1f} h in one process, for cells with timed rows only; "
...
def status(tasks, files, *, n_init, n_iter, seed_base) -> None:
    ...
    print(f"done: {sum(v[0] for v in per_cell.values())} / {len(tasks)}  (files: {', '.join(str(f) for f in files if f.exists())})")
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser = argparse.ArgumentParser(prog="python -m lsab.sweep", description="Run the lengthscale A/B matrix.")
    parser.add_argument("--out", type=Path, required=True)
    parser.add_argument("--datasets", nargs="+", default=list(DEFAULT_DATASETS), choices=names())
    parser.add_argument("--reps", nargs="+", default=list(DEFAULT_REPS), choices=list(featurize.REPS))
    parser.add_argument("--reductions", nargs="+", default=["decorr0.7", "pca64"], choices=list(REDUCTIONS))
    parser.add_argument("--rules", nargs="+", default=["chen", "geom"], choices=list(RULES))
    parser.add_argument("--prior-modes", nargs="+", default=["match_concentration", "match_parameterisation"],
                        choices=PRIOR_MODES)
    parser.add_argument("--cv", type=float, default=0.3)
    parser.add_argument("--seeds", type=int, default=10)
    parser.add_argument("--seed-offset", type=int, default=0)
    parser.add_argument("--iter", type=int, default=50, help="total experiments per campaign, initial design included")
    parser.add_argument("--init", type=int, default=5)
    parser.add_argument("--seed-base", type=int, default=1337)
    parser.add_argument("--shard", default=None, help="i/k: run the i-th of k shards, into its own file")
    parser.add_argument("--threads", type=int, default=1, help="torch threads; run k shards on k cores instead")
    parser.add_argument("--include-degenerate", action="store_true")
    parser.add_argument("--device", default="cpu", help="for embedding molecules not cached yet")
    parser.add_argument("--cache-dir", default=None)
    parser.add_argument("--dry-run", action="store_true")
    parser.add_argument("--status", action="store_true")
    args = parser.parse_args(argv)
    tasks = task_list(args.datasets, args.reductions, args.reps, args.rules, args.prior_modes,
                      args.cv, args.seeds, args.seed_offset)
    ...
    print(f"threads: {torch.get_num_threads()}  output: {out}", flush=True)
    sweep(tasks, out, include_degenerate=args.include_degenerate, device=args.device,
          cache_dir=args.cache_dir, dry_run=args.dry_run, done_from=files + ([out] if out not in files else []), **design)
